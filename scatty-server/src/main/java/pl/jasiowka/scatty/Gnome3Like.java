...
public class Gnome3Like {
	...
	private static final String WRITE_BINDINGS = CMD + S + WR + S + SCHEMA
			+ KEY + " \"<VALUES>\"";
	private static final String BINDING = SCHEMA + KEY + "/<BINDING>/";
	private static final String ATTR_NAME = CMD + S + WR + S + BINDING
			+ "name '<VALUE>'";
	private static final String ATTR_BINDING = CMD + S + WR + S + BINDING
			+ "binding '<VALUE>'";
	private static final String ATTR_COMMAND = CMD + S + WR + S + BINDING
			+ "command '<VALUE>'";
	...
	private void writeBindings(KeyBindings kb) throws IOException {
		...
		shellCommand(ATTR_NAME.replace("<BINDING>", kb.newBindingId).replace(
				"<VALUE>", kb.newBindingName));
		shellCommand(ATTR_BINDING.replace("<BINDING>", kb.newBindingId)
				.replace("<VALUE>", kb.newBindingBinding));
		shellCommand(ATTR_COMMAND.replace("<BINDING>", kb.newBindingId)
				.replace("<VALUE>", kb.newBindingCommand));
		// System.out.println(WRITE_BINDINGS.replace("<VALUES>", kb.bindings));
		// System.out.println(ATTR_NAME.replace("<BINDING>",
		// kb.newBindingId).replace("<VALUE>", kb.newBindingName));
		// System.out.println(ATTR_BINDING.replace("<BINDING>",
		// kb.newBindingId).replace("<VALUE>", kb.newBindingBinding));
		// System.out.println(ATTR_COMMAND.replace("<BINDING>",
		// kb.newBindingId).replace("<VALUE>", kb.newBindingCommand));
	}

	private KeyBindings updateBindings(KeyBindings kb) throws IOException,
			IllegalArgumentException {
		kb.newBindingId = "custom" + kb.bindings.split("[,]").length;
		kb.bindings = kb.bindings.replace("]",
				", '" + BINDING.replace("<BINDING>", kb.newBindingId) + "']");
		return kb;
	}

	public void registerNewKeyBinding(String name, String binding,
